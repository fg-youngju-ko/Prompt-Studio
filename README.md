...
- **아이디어로 시작하기** — "가계부 앱 만들고 싶어" 같은 한 줄 아이디어를 입력하면,
  Google Gemini API가 기획을 구체화할 역질문을 만들어줍니다. 아이디어에 이미 답이
  드러난 항목은 자동으로 채워집니다. 답변을 정리하면 자연스러운 문장으로 요약된
  프롬프트가 만들어집니다. (Gemini API 키 필요, 아래 참고)
...
`generativelanguage.googleapis.com`(Google Gemini API)으로 직접 요청을 보냅니다.
이 요청은 사용자의 브라우저와 Google 사이에서만 이루어지고, 다른 어떤 서버도
거치지 않습니다.
...
"아이디어로 시작하기" 화면에서 API 키를 입력하고 저장하면 됩니다. 무료 등급으로
발급받을 수 있습니다.
...
GitHub Pages로 배포되어 있습니다: **https://fg-youngju-ko.github.io/Prompt-Studio/**

`main` 브랜치에 push하면 자동으로 재배포됩니다. 별도 빌드 과정은 없습니다.
